...
def save_attack_to_json(json_paths_file, n_most_used_edges, complete_mtrx, og_mtrx, name):
...
class Attacker():
    ...
    def scale_naive_attack(self, traffic_matrix, iterations, strength, tag=""):
        ...
        for i in range(iterations + 1):
            ...
            for target_edge_source, target_edge_destination in self.n_most_used_edges:
            ...
            if num_targets == 1:  # put target link in file name.
                ...
                attack_iteration_matrix = "{0}_targets_{1}_strength_{2}".format(traffic_matrix[:-4],
                                                                                num_targets, i)

            # Save data in plain text
            print("Saving data to: {}".format(attack_iteration_matrix))
            np.savetxt(attack_iteration_matrix,
        ...
        result_file = "{}_{}x{}Gbps_{}_{}.txt".format(
            traffic_matrix[:-4], num_targets, int(strength/10**9), iterations, tag)

        print("Saving data to: {}".format(result_file))
        np.savetxt(result_file, new_matrix.astype(int), fmt='%i')

    def one_shot_sustained_attack(self, traffic_matrix, strength, tag="", max_link_util=inf):
        ...
        for i in range(length):
            ...
            json_file = "{0}_targets_{1}_iteration_{2}_strength_{3}".format(traffic_matrix[:-4],
                                                                            num_targets, i, int(strength/10**9))

            save_attack_to_json(
                self.json_path_file, self.n_most_used_edges, new_matrix[i], og_matrix, json_file)
        ...
        '''
        result_file = "{}_{}x{}Gbps_{}_{}.txt".format(
            traffic_matrix[:-4], num_targets, int(strength/10**9), length, tag)
        print("Saving data to: {}".format(result_file))
        np.savetxt(result_file, new_matrix.astype(int), fmt='%i')

    def make_coremelt_attack_matrix(self, n_hosts: int, n_targets: int, strength: int, max_link_util=inf, target_edges=None, save_matrix=True):
        ...
        if save_matrix: 
            attack_file = "./data/traffic/{0}_targets_{1}_strength_{2}".format(self.network, n_targets, int(strength/10**9))
            save_mtrx_to_txt(new_matrix, attack_file)

        return (new_matrix.astype(int), target_edges)

    def make_crossfire_attack_matrix(self, n_hosts, strength, target_node=None, max_link_util=inf, save_matrix=True) -> tuple:
        ...
        attack_file = "data/traffic/{0}_crossfire_targets_{1}_strength_{2}".format(self.network, n_targets, int(strength/10**9))
        if save_matrix:
            save_mtrx_to_txt(new_matrix, attack_file)
        return new_matrix.astype(int), target_edges

    def coremelt_rolling_attack(self, n_hosts:int, n_rounds:int, attack_period:int, calm_period:int):
        ...
        tm = tm.matrix.reshape(tm.num_epochs(), tm.num_nodes() ** 2 ).astype(int)
        ...
        result_file = "./data/traffic/{}_rolling_attack_{}_round.txt".format(self.network, n_rounds)
    ...
    def mixed_rolling_attacks(self):
        ...
        avg_demand = 10**6
        tm = modulated_gravity_tm(self.n_hosts, 12*60, avg_demand, diurnal_freq=freq, pm_ratio = 1.05)
        tm = tm.matrix.reshape(tm.num_epochs(), tm.num_nodes() ** 2 ).astype(int)
        s_time = lambda i: "{}m:{}s".format(i//60, i%60)
        attack_metadata_file = path.join(SCRIPT_HOME,
                                         'data',
                                         'traffic',
            ...
            for e in range(epochs):
                ...
                if e < spiffy_start:
                    ...
                    if spiffy_tm is None:
                        spiffy_tm, spiffy_links = (self.make_coremelt_attack_matrix(self.n_hosts, 1, str))
                    ...
                    if cf_tm is None:
                        cf_tm, cf_links = (self.make_crossfire_attack_matrix(self.n_hosts, attack_lower_bound))
                    
                    tm[e] += cf_tm
                    
                    fob.write("crossfire,\t{},\t{}\n".format(attack_lower_bound, cf_links))
                    ...
                    if cm1_tm is None:
                        cm1_tm, cm1_links = (self.make_coremelt_attack_matrix(self.n_hosts, 2, attack_upper_bound))
                    
                    tm[e] += cm1_tm
                    fob.write("coremelt,\t{},\t{}\n".format(attack_upper_bound, cm1_links))
                    ...
                    if cm2_tm is None:
                        cm2_tm, cm2_links = (self.make_coremelt_attack_matrix(self.n_hosts, 3, attack_upper_bound))
                    
                    tm[e] += cm2_tm
                    fob.write("coremelt,\t{},\t{}\n".format(attack_upper_bound, cm2_links))
                    ...
                    if cm3_tm is None:
                        cm3_tm, cm3_links = (self.make_coremelt_attack_matrix(self.n_hosts, 2, attack_upper_bound))
                    
                    tm[e] += cm3_tm
                    fob.write("coremelt,\t{},\t{}\n".format(attack_upper_bound, cm3_links))
                    ...
                    if cm4_tm is None:
                        cm4_tm, cm4_links = (self.make_coremelt_attack_matrix(self.n_hosts, 4, attack_lower_bound))
                    
                    tm[e] += cm4_tm
                    fob.write("coremelt,\t{},\t{}\n".format(attack_lower_bound, cm4_links))
                    ...
                    if cm5_tm is None:
                        cm5_tm, cm5_links = (self.make_coremelt_attack_matrix(self.n_hosts, 3, attack_lower_bound))
                    
                    tm[e] += cm5_tm
                    fob.write("coremelt,\t{},\t{}\n".format(attack_lower_bound, cm5_links))
                    
                else: 
                    fob.write("None,\t0,\t[]\n")

        save_mtrx_to_txt(tm, "./data/traffic/rolling_attack/{}-rolling-mixed-type-attack.txt".format(self.network))
...
if __name__ == "__main__":
    ...
    if 0:  # Experiment to scale traffic on a heavily shared link.
        ...
        for network in networks:
            test_paths = SCRIPT_HOME + "/data/results/{0}_add_circuit_heuristic_10/__0/paths/ecmp_0".format(
                network)
            test_matrix = SCRIPT_HOME + "/data/traffic/{}.txt".format(
                network)
    ...
    if 0:  # Experiment to scale traffic on Ripple's target edges.
        test_paths = SCRIPT_HOME + "/data/results/bellCanada_add_circuit_heuristic_10/__0/paths/ecmp_0"
    ...
    if 1: # Coremelt Every Link!
        from utilities.tmg import rand_gravity_matrix
        network = "CRL"
        # benign_traffic = 100E9
        benign_traffic = 100E9 
        attack_traffic = 300E9 
        test_paths = SCRIPT_HOME + "/data/paths/CRL.json"
        N_HOSTS=33

        attacker = Attacker(network, test_paths, n_hosts=N_HOSTS)        
        attacker.find_target_link()
        base_matrix = rand_gravity_matrix(N_HOSTS, 1, benign_traffic, SCRIPT_HOME + '/data/traffic/{}_base_tm_{:.2e}_gravity.txt'.format(network, benign_traffic))
        n_edges = len(attacker.edge_flows.keys())
        continuous_series = np.tile(base_matrix, (n_edges + 1, 1)) # used to evaluate ECMP and MCF without ONSET.
        for i, edge in enumerate(attacker.edge_flows.keys()):
            print(i+1, edge)
            attack_matrix, attacked_edges = attacker.make_coremelt_attack_matrix(N_HOSTS, 1, attack_traffic, benign_traffic, [edge], save_matrix=False)
            continuous_series[i+1] += attack_matrix
        outfile = SCRIPT_HOME + "/data/traffic/{}_coremelt_every_link_{:.2e}.txt".format(network, attack_traffic)
    ...
    if 0: # Crossfire Every Node!
        ...
        outfile = SCRIPT_HOME + '/data/traffic/' + network + '_base_tm_' + ATTACK_STRENGTH_STRING + '_gravity.txt'
        base_matrix = rand_gravity_matrix(N_HOSTS, 1, BACKGROUND_VOLUME , outfile)
        
        G = nx.Graph()
        G.add_edges_from(attacker.edge_flows.keys())
        node_list = [ 's' + str(i+1) for i in range(len(G.nodes())) ]
        n_nodes = len(node_list)
        continuous_series = np.tile(base_matrix, (n_nodes + 1, 1)) # used to evaluate ECMP and MCF without ONSET. 
        for i, node_i in enumerate(node_list):
            print(i+1, node_i, end=' ')
            attack_matrix, attacked_edges = attacker.make_crossfire_attack_matrix(N_HOSTS, ATTACK_STRENGTH, node_i, MAX_LINK_UTIL, save_matrix=False )
            print(attacked_edges)
            continuous_series[i+1] += attack_matrix

        outfile = SCRIPT_HOME + "/data/traffic/" + network + "_crossfire_every_node_" + ATTACK_STRENGTH_STRING + ".txt"
        print("Saving traffic matrix to: {}".format(outfile))
        np.savetxt(outfile, continuous_series.astype(int), fmt='%i')
