...
class AlpWolf():
    ...
    def __init__(self, base_graph: Graph, fallow_transponders=5) -> None:
        self.circuit_bandwidth = 10  # Gb/s
        self.transponders_per_degree = 10
        self.base_graph = Graph.copy(base_graph)
    ...
    def _init_transponders(self):
        ...
        Each for getAvailableTransponder and addCircuit to handle mapping of tranponders to 
        pair nodes on each circuit. 

        Raises:
            Exception: If it cannot add a circuit due to transponder constraints then it fails. 
        """
        # The transponder node-attribute is a dictionary (transponder ID -> node). node -1 implies transponder is unassigned.
    ...
    def get_available_transponder(self, t_dict: dict) -> int:
        ...
        If no transponder is availabe, returns -1. 
    ...
    def add_circuit(self, u: str, v: str, transponder_u: int = None, transponder_v: int = None) -> int:
        ...
        if transponder_u == -1 or transponder_v == -1:
            logger.error("Couldn't add circuit. Transponder pair unavailable")
            return -1

        # Make sure tranponders are currently unassigned.
    ...
    def get_candidate_circuits(self, candid_set="all", k=0, l=0) -> list:
        ...
        if candid_set == "all":
            ...
            #     if len(set([c1[0], c1[1], c2[0], c2[1]])) == 4:
            #         candidates.append()
            return candidates
