...
from networkx import read_gml, Graph
...
sys.path.append(path.abspath('./src/utilities'))
from src.ONSET_MILP import Link_optimization
from src.utilities import SCRIPT_HOME, USER_HOME
from src.utilities.plot_reconfig_time import get_reconfig_time
from src.utilities.tmg import rand_gravity_matrix
from src.utilities.logger import logger, formatter
from src.utilities.sysUtils import count_lines, get_edge_congestion
from src.utilities.gml_to_dot import Gml_to_dot
from src.utilities.write_gml import write_gml
from src.utilities.diff_compare import diff_compare
from src.utilities.plotters import cdf_average_congestion, cdf_churn, draw_graph, plot_points, congestion_heatmap
from src.utilities.post_process import post_process, post_proc_timeseries, read_result_val
...
class Attack_Sim():
    def __init__(self, name: str, test: str, iterations=0, te_method='-ecmp', start_clean=False, magnitude=100*10**10, traffic_file="", strategy="", fallow_transponders=5, use_heuristic="", method="all", congestion_threshold_upper_bound=0.8,
                 congestion_threshold_lower_bound=0.3, proportion="", shakeroute=False):
        self.nonce = "./temp/" + sha1("{}{}{}{}{}{}{}{}{}{}{}{}{}{}".format(*[str(s) for s in [name, test, iterations, te_method,
                                      start_clean, magnitude, traffic_file,
                                      strategy, fallow_transponders, use_heuristic, method, congestion_threshold_upper_bound, congestion_threshold_lower_bound,  proportion]]).encode()).hexdigest()
        print("Nonce: ", self.nonce)
        logger.info("Initializing simulator: {} {} {}".format(name, test, iterations))
        self.name = name
        self.test = test
        ...
        self.wolf = AlpWolf(self.base_graph.G, self.fallow_transponders)
    ...
    def _yates(self, topo_file, result_path, traffic_file=""):
        ...
        command_args = [USER_HOME + "/.opam/4.06.0/bin/yates", topo_file, traffic_file, traffic_file,
                        self.hosts_file, self.te_method, "-num-tms", "1", "-out", result_path,
                        ]
    ...
    def perform_sim(self, circuits=1, start_iter=0, end_iter=0):
        if end_iter == 0: 
        ...
        flux_circuits = [] # Stateful list of active circuits triggered from sig_add_circutes
        ...
        with open(traffic, 'rb') as fob:
            ...
            for i in range(iterations):
                ITERATION_ID = name+"_{}-{}".format(i+1, iterations)
                ...
                try:
                    ...
                    if self.strategy == 'cli':
                        ...
                        if type(new_circuit) == tuple and len(new_circuit) == 2:
                            print("Adding {} ({}, {}) circuits.".format(circuits), *new_circuit)
                    ...
                    # elif self.strategy == 'optimal' and sig_add_circuits and not circuits_added:
                    elif self.strategy == 'optimal' and sig_add_circuits:
                    ...
                    return_data["Strategy"].append("{} {}".format(self.te_method, self.strategy))
                    return_data["Routing"].append("{}".format(self.te_method).strip("-").upper())
                    ...
                    return_data["Congestion"].append(
                        read_result_val(
                            path.join(
                                ...
                                "data",
                                "results",
                                ITERATION_REL_PATH, "MaxExpCongestionVsIterations.dat")
                        )
                    )
                    ...
                    return_data["Total Links Dropped"].append(len(drop_circuits))
    ...
    def validate_yates_params(self):
        ...
        def verify_topo():
            ...
            if self.shakeroute:
                base_topo_file = path.join(SCRIPT_HOME, "data", "graphs", "shakeroute", self.shakeroute, name+".gml")
            
            else:
                base_topo_file = path.join(SCRIPT_HOME, "data", "graphs", "gml", name+".gml")
            ...
            self.base_graph.import_gml_graph(base_topo_file)
        ...
        def verify_hosts():
            ...
            if self.shakeroute: 
                hosts_file = path.join(SCRIPT_HOME, "data", "hosts", self.shakeroute+".hosts")
            else:
                hosts_file = path.join(SCRIPT_HOME, "data", "hosts", name+".hosts")
            ...
            try:  # sets self.num_hosts and checks file exists.
                ...
                def create_host_file():
                    ...
                    makedirs(hosts_folder, exist_ok=True)
        ...
        def verify_traffic():
            ...
            try:
                ...
                logger.debug("traffic file linecount passed.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("test", type=str,
                        help="str: Type of test to run. Either 'add_circuit' to do preprocessing or anyting else to run another experiment.")
    ...
    parser.add_argument("-tsLables", "--time_series_labels", nargs="+",
                        help="Required if -ts set. Time series data lables.")
    ...
    attack_sim = Attack_Sim(topology, test, iterations=iterations,
                            te_method='-ecmp',
